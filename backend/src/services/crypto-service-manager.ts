    ...
    for (let attempt = 0; attempt < this.maxRetries; attempt++) {
      ...
      for (let i = 0; i < this.services.length; i++) {
        ...
        try {
          ...
          logger.error(
            `Error with ${service.getName()}: ${error instanceof Error ? error.message : 'Unknown error'}`
          );
      ...
      if (attempt < this.maxRetries - 1) {
        logger.warn(
          `All services failed, attempt ${attempt + 1}/${this.maxRetries}. Waiting before retry...`
        );
